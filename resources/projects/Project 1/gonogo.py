...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\My Drive\\Psychology\\PsychoPy Tutorial\\resources\\projects\\Project 1\\gonogo.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    end_of_block_text = visual.TextStim(win=win, name='end_of_block_text',
        text='',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    blocks = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('conditions_files/blocks.xlsx'),
    ...
    for thisBlock in blocks:
        ...
        _instr_key_allKeys = []
        # keep track of which components have finished
        instrComponents = [instr_text, instr_key]
        ...
        while continueRoutine:
            ...
            if instr_key.status == STARTED and not waitOnFlip:
                theseKeys = instr_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=True)
        ...
        if instr_key.keys in ['', [], None]:  # No response was made
            instr_key.keys = None
        blocks.addData('instr_key.keys',instr_key.keys)
        if instr_key.keys != None:  # we had a response
            blocks.addData('instr_key.rt', instr_key.rt)
            blocks.addData('instr_key.duration', instr_key.duration)
        ...
        for thisTrial in trials:
            ...
            trial_polygon.setFillColor(colour)
            trial_polygon.setLineColor(colour)
            trial_resp.keys = []
            trial_resp.rt = []
            _trial_resp_allKeys = []
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 1.5:
                ...
                if trial_polygon.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > trial_polygon.tStartRefresh + 1.5-frameTolerance:
                        # keep track of stop time/frame for later
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('end_of_block.started', globalClock.getTime())
        end_of_block_text.setText("End of block " + str(block))
        # keep track of which components have finished
        end_of_blockComponents = [end_of_block_text]
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # if end_of_block_text is starting this frame...
            if end_of_block_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                end_of_block_text.frameNStart = frameN  # exact frame index
                end_of_block_text.tStart = t  # local t and not account for scr refresh
                end_of_block_text.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(end_of_block_text, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'end_of_block_text.started')
                # update status
                end_of_block_text.status = STARTED
                end_of_block_text.setAutoDraw(True)
            
            # if end_of_block_text is active this frame...
            if end_of_block_text.status == STARTED:
                # update params
                pass
            
            # if end_of_block_text is stopping this frame...
            if end_of_block_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > end_of_block_text.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
                    end_of_block_text.tStop = t  # not accounting for scr refresh
                    end_of_block_text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'end_of_block_text.stopped')
                    # update status
                    end_of_block_text.status = FINISHED
        ...
        for thisComponent in end_of_blockComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('end_of_block.stopped', globalClock.getTime())
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
