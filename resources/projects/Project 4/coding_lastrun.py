...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\My Drive\\Psychology\\PsychoPy Tutorial\\resources\\projects\\Project 4\\coding_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    nCrosses = int(expInfo['how many crosses?'])
    ...
    for thisTrials_loop in trials_loop:
        ...
        # keep track of which components have finished
        trialComponents = [trial_slider, trial_mouse, trial_textbox, trial_reset, trial_reset_text]
        ...
        while continueRoutine:
            ...
            clockText = "Time since start: " +\
            str(currentTime) +\
            "\nSince refresh: " +\
            str(sinceReset) +\
            "\nPosition: "
            # Run 'Each Frame' code from trial_clock_PY
            clockText += str(round(trial_slider.getMarkerPos(), 5))
            # Run 'Each Frame' code from trial_trig_PY
            ...
            for i in range(len(crosses)):
                ...
                crossPositions.append([radius * sin((((i * angle) + ori)%360)* pi / 180), radius * cos((((i * angle) + ori)%360) * pi / 180)])
            ...
            if trial_textbox.status == STARTED:
                # update params
                trial_textbox.setText(clockText, log=False)
